...
| Inlay hints                    | `textDocument/inlayHint`                                   | ✅ merged 2026-09-09 (PR #617); SDK gate 3.14.0-139.0.dev. Stage 2 (#622, per-category options) implemented on branch `lsp-inlay-hints-options` (2026-09-19, not yet a PR): adds `workspace/configuration` (server→client pull) + `workspace/didChangeConfiguration` (client→server, as legacy `lsp.notification`), SDK CL 546841 (sdk#64101, protocol 1.41.0) **landed 2026-09-22** as dart-lang/sdk@6700ccc4316; sandbox-tested; plugin PR #679 open, waits for the first dev SDK containing it |
