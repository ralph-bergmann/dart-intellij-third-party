...
|---|---|---|
| S1 | **Implement now — Stack 1:** PR A = inlay hints (Part 3 of the 2026-07-30 plan, `MIN_LSP_INLAY_HINTS_SDK_VERSION = "3.14.0-139.0.dev"`), PR B = `textDocument/typeDefinition` (#580) — superseded: helin24 had already opened #615 for it (2026-08-18); my #618 was closed as a duplicate on 2026-08-19. Lesson: re-list open PRs right before starting a feature. | Both are the proven 4-file pattern (bridge override + capability + `LspMethod` + customizer), no SDK work left, no legacy code to gate, no PSI arbitration problem (§2.3). Identical shape → reviewers see one pattern; stacking keeps Ralph at ≤ 2 open PRs. |
